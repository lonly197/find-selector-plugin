...
(function($) {
   ...
   *    (http://plugins.learning$.com/cluetip/), which in turn was inspired
   ...
   * http://plugins.$.com/project/bgiframe - BeautyTips will automatically
   ...
   * $(selector).ct();
   * $(selector).ct('Content text');
   * $(selector).ct('Content text', {option1: value, option2: value});
   * $(selector).ct({option1: value, option2: value});
   *
   * For more/better documentation and lots of examples, visit the demo page included with the distribution
   *
   */

  $.fn.ct = function(content, options) {
    ...
    if ($.fn.hoverIntent && $.ct.defaults.trigger == 'hover') {
      $.ct.defaults.trigger = 'hoverIntent';
    }

    return this.each(function(index) {

      var opts = $.extend(false, $.ct.defaults, $.ct.options, options);
       ...
       * These are the init functions to handle the .ct() call
       */
      ...
      if (opts.killTitle) {
        ...
        $(this).find('[title]').andSelf().each(function() {
          if (!$(this).prop('ct-xTitle')) {
            $(this).prop('ct-xTitle', $(this).prop('title')).prop('title', '');
          }
        });
       ...
       */
      this.ctOn = function (preserveOrginal) {
        preserveOrginal = preserveOrginal || false;
        if (typeof $(this).data('ct-box') == 'object') {
          // if there's already a popup, remove it before creating a new one.
          if(preserveOrginal==true){
            this.orgContent = $("<div/>").appendTo(document.body);
            $($(this).data('ct-box').find(".ct-content").get(0).firstChild).appendTo(this.orgContent);
          }
          this.ctOff();
        }
        ...
        // turn off other tips
        $($.ct.vars.closeWhenOpenStack).ctOff();

        // add the class to the target element (for hilighting, for example)
        // ct-active is always applied to all, but activeClass can apply another
        $(this).addClass('ct-active ' + opts.activeClass);

        if (contentSelect && opts.ajaxPath == null) {
          // bizarre, I know
          if (opts.killTitle) {
            // if we've killed the title attribute, it's been stored in 'ct-xTitle' so get it..
            $(this).prop('title', $(this).prop('ct-xTitle'));
          }
        ...
        if (opts.ajaxPath != null && content == false) {
          ...
          var cacheData = opts.ajaxCache ? $(document.body).data('ctCache-' + url.replace(/\./g, '')) : null;
            ...
            var ajaxOpts = $.extend(false,
            {
              ...
              complete: function(XMLHttpRequest, textStatus) {
                if (textStatus == 'success' || textStatus == 'notmodified') {
                  if (opts.ajaxCache) {
                    $(document.body).data('ctCache-' + url.replace(/\./g, ''), XMLHttpRequest.responseText);
                  }
                  ajaxTimeout = false;
                  content = selector ?
        ...
        var width = $(this).ctOuterWidth();
        var height = $(this).outerHeight();

        if(preserveOrginal==true && typeof this.orgContent=='object'){
          content = this.orgContent.get(0).firstChild;
          ...
          var origClones = $(original).data('ct-clones') || [];
          origClones.push(clone);
          $(original).data('ct-clones', origClones);
          $(clone).data('ct-orig', original);
          $(this).data('ct-content-orig', {original: original, clone: clone});
          content = clone;
        }
        ...
        var $text = $('<div class="ct-content"></div>').append(content).css({padding: opts.padding, position: 'absolute', width: (opts.shrinkToFit ? 'auto' : opts.width), zIndex: opts.textzIndex, left: shadowShiftX, top: shadowShiftY}).css(opts.cssStyles);
        // create the wrapping box which contains text and canvas
        // put the content in it, style it, and append it to the same offset parent as the target
        var $box = $('<div class="ct-wrapper"></div>').append($text).addClass(opts.cssClass).css({position: 'absolute', width: opts.width, zIndex: opts.wrapperzIndex, visibility:'hidden'}).appendTo(offsetParent);

        //remove tmp div
        if(typeof this.orgContent=='object' && this.orgContent!==null){
          this.orgContent.remove();
          this.orgContent = null;
        }

        opts.postContent.apply(this, [$text[0], $box[0]]);

        // use bgiframe to get around z-index problems in IE6
        // http://plugins.$.com/project/bgiframe
        if ($.fn.bgiframe) {
          $text.bgiframe();
          $box.bgiframe();
        }

        $(this).data('ct-box', $box);
        ...
        var textOutWidth = numb($text.ctOuterWidth());
        ...
        switch(position) {
          ...
          case 'top':
            ...
            textRightSpace = (winRight - opts.windowMargin) - ($text.offset().left + $text.ctOuterWidth(true));
            ...
            textTop = $text.ctPosition().top + numb($text.css('margin-top'));
            textLeft = $text.ctPosition().left + numb($text.css('margin-left'));
            textRight = textLeft + $text.ctOuterWidth();
            textBottom = textTop + $text.outerHeight();
            textCenter = {x: textLeft + ($text.ctOuterWidth()*opts.centerPointX), y: textTop + ($text.outerHeight()*opts.centerPointY)};
          ...
          case 'left':
            ...
            $box.css({top: vert + 'px', left: ((left - $text.ctOuterWidth(true)) + opts.overlap) + 'px'});
        ...
        $(canvas).prop('width', (numb($text.ctOuterWidth(true)) + opts.strokeWidth*2 + shadowMarginX)).prop('height', (numb($text.outerHeight(true)) + opts.strokeWidth*2 + shadowMarginY)).appendTo($box).css({position: 'absolute', zIndex: opts.boxzIndex});
        ...
        if (opts.overlay) {
          ...
          var overlay = $('<div class="ct-overlay"></div>').css({
              position: 'absolute',
        ...
        if (opts.clickAnywhereToClose) {
          $.ct.vars.clickAnywhereStack.push(this);
          $(document).click($.ct.docClick);
        }

        // stick this element into the closeWhenOthersOpen stack
        if (opts.closeWhenOthersOpen) {
          $.ct.vars.closeWhenOpenStack.push(this);
        }
      ...
      this.ctOff = function() {
        ...
        i.ctCleanup = function(){
          var box = $(i).data('ct-box');
          var contentOrig = $(i).data('ct-content-orig');
          var overlay = $(i).data('ct-overlay');
          if (typeof box == 'object') {
            $(box).remove();
            $(i).removeData('ct-box');
          }
          if (typeof contentOrig == 'object') {
            var clones = $(contentOrig.original).data('ct-clones');
            $(contentOrig).data('ct-clones', arrayRemove(clones, contentOrig.clone));
          }
          if (typeof overlay == 'object') {
            $(overlay).remove();
            $(i).removeData('ct-overlay');
          }

          // remove this from the stacks
          $.ct.vars.clickAnywhereStack = arrayRemove($.ct.vars.clickAnywhereStack, i);
          $.ct.vars.closeWhenOpenStack = arrayRemove($.ct.vars.closeWhenOpenStack, i);

          // remove the 'ct-active' and activeClass classes from target
          $(i).removeClass('ct-active ' + opts.activeClass);
        ...
        opts.hideTip.apply(this, [box, i.ctCleanup]);

      }; // </ turnOff() >

      var refresh = this.ctRefresh = function() {
        this.ctOff();
        this.ctOn();
      };
   ...
   *  and .ctPosion() can be replaced with .position() above...
   */
  $.fn.ctPosition = function() {

    function num(elem, prop) {
      return elem[0] && parseInt( $.css(elem[0], prop, true), 10 ) || 0;
    };
  ...
  */
  ...
  /**
   * A convenience function to run ctOn() (if available)
   * for each selected item
   */
  $.fn.ctOn = function(preserveOrginal) {
    return this.each(function(index){

      if ($.isFunction(this.ctOn)) {
        this.ctOn(preserveOrginal);
      }
  ...
  /**
   *
   * A convenience function to run ctOff() (if available)
   * for each selected item
   */
  $.fn.ctOff = function() {

    return this.each(function(index){
      if ($.isFunction(this.ctOff)) {
        this.ctOff();
      }
    });
  }; // </ $().ctOff() >
  ...
  $.ct.vars = {clickAnywhereStack: [], closeWhenOpenStack: []};
  ...
  $.ct.docClick = function(e) {
    ...
    if (!$(e.target).parents().andSelf().filter('.ct-wrapper, .ct-active').length && $.ct.vars.clickAnywhereStack.length) {
      // if clicked element isn't inside tip, close tips in stack

      $($.ct.vars.clickAnywhereStack).ctOff();
      $(document).unbind('click', $.ct.docClick);
    }
  }; // </ docClick() >
   ...
   *
   *   $.ct.options.width = 400;

   *
   * Be sure to use *$.ct.options* and not $.ct.defaults when overriding
   *
  ...
  $.ct.defaults = {
    trigger:         'hover',                // trigger to show/hide tip
                                             ...
                                             //   'none'            // use $('#selector').ctOn(); and ...ctOff();
    ...
    killTitle:        false,                  // kill title tags to avoid double tooltips
    ...
    fill:             "#FFFFA9",  // fill color for the tooltip box, you can use any CSS-style color definition method
                                             // http://www.w3.org/TR/css3-color/#numerical - not all methods have been tested

    windowMargin:     10,                    // space (px) to leave between text box and browser edge

    strokeWidth:      1,                     // width of stroke around box, **set to 0 for no stroke**
    strokeStyle:      "#999",                // color/alpha of stroke
    ...
    activeClass:      'ct-active',           // class added to TARGET element when its BeautyTip is active
    ...
    ajaxOpts:         {},                    // any other ajax options - timeout, passwords, processing functions, etc...
                                             // see http://docs.$.com/Ajax/$.ajax#options

    preBuild:         function(){},          // function to run before popup is built
    postContent:      function(){},          // function to run before popup is built
    ...
    hoverIntentOpts:  {                          // options for hoverIntent (if installed)
                        interval: 300,           // http://cherne.net/brian/resources/$.hoverIntent.html
                        timeout: 500
                      },
    preserveOrginal: false
